...
def parse_workflow_log(log_path: Path) -> Dict:
    ...
    # Verify total_files_processed by counting actual descriptions
    # This is crucial for resumed workflows where the log only shows the last run
    actual_count = count_actual_descriptions(workflow_dir_path)
    if actual_count is not None and actual_count != stats['total_files_processed']:
        if actual_count > stats['total_files_processed']:
            # Actual count is higher - likely a resumed workflow
            # Use the actual count as the authoritative value
            print(f"  Note: Adjusted count from {stats['total_files_processed']} to {actual_count} (resumed workflow)")
            stats['total_files_processed'] = actual_count
        elif stats['total_files_processed'] == 0:
            # Log had no count but we found actual descriptions
            stats['total_files_processed'] = actual_count
    
    return stats


def count_actual_descriptions(workflow_dir: Path) -> Optional[int]:
    """
    Count actual descriptions generated by checking the progress file or description file.
    This provides the authoritative count, especially important for resumed workflows.
    
    Args:
        workflow_dir: Path to the workflow directory
        
    Returns:
        Number of descriptions, or None if cannot be determined
    """
    # Priority 1: Check image_describer_progress.txt (most reliable)
    # This file accumulates across all runs including resumes
    progress_file = workflow_dir / "logs" / "image_describer_progress.txt"
    if progress_file.exists():
        try:
            with open(progress_file, 'r', encoding='utf-8') as f:
                # Each line represents one completed description
                count = sum(1 for line in f if line.strip())
                return count
        except Exception as e:
            print(f"Warning: Could not read progress file {progress_file}: {e}")
    
    # Priority 2: Check image_descriptions.txt
    # Count separator lines (====) which indicate distinct descriptions
    desc_file = workflow_dir / "image_descriptions.txt"
    if desc_file.exists():
        try:
            ...
            print(f"Warning: Could not read descriptions file {desc_file}: {e}")
...
Examples:
  ...
  idt stats --input-dir /data/workflows --csv-output my_stats.csv
    ...
    for workflow_dir in workflow_dirs:
        ...
        if log_files:
            # If multiple workflow logs exist (e.g., from --resume), select the best one
            # Priority: 1) Log with end_time (completed), 2) Most recent by filename timestamp
            if len(log_files) > 1:
                # Quick scan to find logs with end_time
                completed_logs = []
                for log_file in log_files:
                    try:
                        with open(log_file, 'r', encoding='utf-8') as f:
                            content = f.read()
                            if 'End time:' in content and 'Total execution time:' in content:
                                completed_logs.append(log_file)
                    except Exception:
                        pass
                
                if completed_logs:
                    # Use the most recent completed log (by filename timestamp)
                    log_file = sorted(completed_logs, reverse=True)[-1]
                else:
                    # No completed logs, use most recent by filename
                    log_file = sorted(log_files, reverse=True)[-1]
                
                print(f"Analyzing: {workflow_dir.name} (selected {log_file.name} from {len(log_files)} logs)")
            else:
                # Only one log file
