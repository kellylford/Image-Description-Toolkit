# Image Description Toolkit v4.6.1
...
|---|---|---|
| **`ImageDescriptionToolkitSetup-4.6.1-windows.exe`** | Windows 10/11, 64-bit | **Start here.** Installs all three applications. |
| **`IDT-4.6.1-macos-arm64.dmg`** | macOS, Apple Silicon | **Start here.** Contains all three. |
| `idt-4.6.1-windows-x64.exe` | Windows | Just the command-line tool, no installer |
| `ImageDescriber-4.6.1-windows-x64.exe` | Windows | Just the image GUI, no installer |
| `IDTChat-4.6.1-windows-x64.exe` | Windows | Just the chat client, no installer |
| `idt-4.6.1-macos-arm64.tar.gz` | macOS | Just the command-line tool |
...
- **Apple Intelligence retries when its model manager drops a request.** Apple's
  ...
  fine on a second attempt — but IDT treated it as final and skipped the image.
  It is now retried, so you should not see it at all.
- **Attaching more than one photo to a message in IDT Chat no longer returns a
  blank answer.** The images were sent at full size and the local server dropped
  the request. They are scaled before sending now.
- **When Apple Intelligence declines an image, it says so in plain words** instead
  of showing a raw server error, and tells you that a different prompt style
  usually works.
