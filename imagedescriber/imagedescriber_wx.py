...
class ImageDescriberFrame(wx.Frame, ModifiedStateMixin):
    ...
    def __init__(self):
        ...
        self.video_desc_worker = None  # Store VideoDescriptionWorker reference
        ...
        if EVT_VIDEO_DESCRIPTION_COMPLETE:
            self.Bind(EVT_VIDEO_DESCRIPTION_COMPLETE, self.on_video_description_complete)
        if EVT_VIDEO_DESCRIPTION_FAILED:
            self.Bind(EVT_VIDEO_DESCRIPTION_FAILED, self.on_video_description_failed)
...
<p class="stats">Total images: {len(described_items)}</p>''')
    ...
    def on_files_discovered(self, event):
        ...
        # With 1183 files in 24 batches, that's O(n2) = tens of thousands of operations
    ...
    def on_describe_video(self, event):
        ...
        # Get selected video or prompt user
        selected_video = None
        if self.current_image_item and self.current_image_item.item_type == "video":
            selected_video = self.current_image_item.file_path
        ...
        api_key = self.get_api_key_for_provider(options['provider'])
        if not api_key and options['provider'] in ['openai', 'claude']:
            show_error(self, f"API key required for {options['provider']}.\n\n"
                       "Set it in Tools > Configure Settings > API Keys.")
            return

        # Create video describer and run in background
        self.SetStatusText(f"Generating video description for {Path(selected_video).name}...", 0)
    ...
    def on_video_description_complete(self, event):
        """Handle video description completion"""
        result = event.result
        video_path = result.get('video_path', '')

        # Add description to video item
        if video_path in self.workspace.items:
            video_item = self.workspace.items[video_path]
            ...
            desc = ImageDescription(
                ...
                prompt_style=result.get('prompt', ''),
                provider=result.get('provider', 'unknown'),
                metadata={
                    'processing_time_seconds': result.get('processing_time', 0),
                    'num_frames_used': result.get('num_frames_used', 0),
                    'video_metadata': result.get('video_metadata', {}),
                    'timestamps': result.get('timestamps', [])
                }
            )
            video_item.add_description(desc)

            self.mark_modified()
            self.refresh_image_list()

            # Update display if this is current item
            if self.current_image_item == video_item:
                self.display_image_info(video_item)

        # Show completion message
        processing_time = result.get('processing_time', 0)
        num_frames = result.get('num_frames_used', 0)
        model = result.get('model', 'unknown')

        show_info(self,
            f"Video description complete!\n\n"
            f"Video: {Path(video_path).name}\n"
            f"Model: {model}\n"
            f"Frames analyzed: {num_frames}\n"
            f"Processing time: {processing_time:.1f}s\n\n"
            f"The description has been added to the video's metadata."
        )

        self.SetStatusText(f"Video described: {Path(video_path).name}", 0)

        # Clear worker reference
        self.video_desc_worker = None

    def on_video_description_failed(self, event):
        """Handle video description failure"""
        error = event.error
        video_path = event.video_path

        show_error(self, f"Video description failed:\n\n{error}")
        self.SetStatusText(f"Video description failed: {Path(video_path).name}", 0)
