...
# Chat event types were removed with ChatProcessingWorker. Chat now delivers
# idt_core.chat events straight to the window via shared/chat_worker_wx.py,
# which needs no wx event registration.
...
# ChatProcessingWorker lived here until August 2026: ~690 lines that hand-rolled
# one streaming SDK call per provider (Ollama, OpenAI, Claude, MLX) plus token
# budgeting, image de-duplication and three wx event types.
#
# All of it now lives in idt_core.chat, shared with `idt chat` and the
# standalone IDT Chat app, and reaches wx through shared/chat_worker_wx.py.
# Deleting it rather than leaving it "just for MLX" was the point: this repo
# already grew three parallel provider layers by adding rather than removing.
