@echo off
REM ============================================================================
REM Install Image Description Toolkit - Universal Installer
REM ============================================================================
...
REM
REM NOTE: All executables work on both AMD64 and ARM64 Windows systems.
REM ============================================================================

echo.
echo ========================================================================
echo IMAGE DESCRIPTION TOOLKIT - INSTALLER
echo ========================================================================
echo.
...
set VIEWER_PACKAGE=
for %%f in (viewer_v*.zip) do set VIEWER_PACKAGE=%%f
if "%VIEWER_PACKAGE%"=="" (
    echo ERROR: Viewer package not found ^(viewer_v*.zip^)
    set /a MISSING_PACKAGES+=1
) else (
...
set PROMPT_PACKAGE=
for %%f in (prompt_editor_v*.zip) do set PROMPT_PACKAGE=%%f
if "%PROMPT_PACKAGE%"=="" (
    echo ERROR: Prompt Editor package not found ^(prompt_editor_v*.zip^)
    set /a MISSING_PACKAGES+=1
) else (
...
for %%f in (imagedescriber_v*.zip) do set DESCRIBER_PACKAGE=%%f
if "%DESCRIBER_PACKAGE%"=="" (
    echo ERROR: ImageDescriber package not found ^(imagedescriber_v*.zip^)
    set /a MISSING_PACKAGES+=1
) else (
...
echo.
echo NOTE: All applications work on both AMD64 and ARM64 Windows systems.
echo.
pause
