...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate consistent alt text across all gallery configurations'
    )
    parser.add_argument(
        '--jsondata-dir',
        type=str,
        default='jsondata',
        help='Directory containing JSON description files (default: jsondata)'
    )
    
    args = parser.parse_args()
    jsondata_dir = Path(args.jsondata_dir)
    
    if not jsondata_dir.exists():
        print(f"Error: Directory not found: {jsondata_dir}")
        print(f"Please specify correct path with --jsondata-dir")
        return 1
    ...
    print(f"Working directory: {jsondata_dir}")
    print()
