...
class ImageDescriptionViewer(QWidget):
    ...
    def display_description(self, row):
        ...
        if 0 <= row < len(self.descriptions):
            ...
            self.description_text.setAccessibleName("Description Text")
            self.description_text.setAccessibleDescription(self.descriptions[row])
        else:
    ...
    def copy_description(self):
        ...
        if 0 <= row < len(self.descriptions):
            ...
            copied = clipboard.text(QClipboard.Mode.Clipboard)
            if copied == self.descriptions[row]:
                QMessageBox.information(self, "Copy Description", "Description copied to clipboard.")
            else:
                QMessageBox.warning(self, "Copy Description", "Failed to copy description.")
        ...
        if 0 <= row < len(self.image_files):
            ...
            if os.path.isfile(img_path):
                ...
                if not image.isNull():
                    ...
                    if not clipboard.image(QClipboard.Mode.Clipboard).isNull():
                        QMessageBox.information(self, "Copy Image", "Image copied to clipboard.")
                    else:
                        QMessageBox.warning(self, "Copy Image", "Failed to copy image.")
                else:
